...
namespace TibiaBotMvvm.Models
{
    public class Heal
    {
        public Heal()
        {
            ...
            List = Keys.Select(p => new Key { Name = p.Key, Value = p.Value }).ToList();
//

//            List = Keys.Select(p => p.Key).ToList();
        }
        public int ResourceFrom { get; set; }
        public int ResourceTo { get; set; }
        ...
        public enum MyPercent
        {
            Percent,
            Flat
        }

        public MyPercent Percent { get; set; }
        ...
        private Dictionary<string, int> Keys { get; } = new Dictionary<string, int>()
        {
            {"F1 key", 0x70},
            {"F2 key", 0x71},
            {"F3 key", 0x72},
